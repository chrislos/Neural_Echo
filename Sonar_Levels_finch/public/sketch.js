...
// ─── FINCH-DEMO: Vogel-Loop mit Kopf-gesteuertem Pitch ───
// Idee: Ein kurzes Fink-Zwitschern (finch.wav) loopt endlos "vor dem inneren
// Auge" (stereo, wandert NICHT im Raum mit). Der Kopf wird zum Abspielregler:
//   ganz links schauen  → Originaltonhöhe (playbackRate = 1)
//   nach rechts drehen  → immer tiefer gepitcht (und dadurch langsamer)
// Der Trick: playbackRate funktioniert wie eine Bandmaschine – Tonhöhe und
// Tempo hängen zusammen. Runtergepitcht hört man plötzlich, dass die zwei
// "Bips" in Wirklichkeit kleine Melodien sind. Vögel dekorieren ihre Töne
// im Millisekunden-Bereich – zu schnell für unser Ohr, bis man es verlangsamt.
let sceneFinchActive = false;         // schaltet die Pitch-Steuerung im tick() an
const soundsFinch = {};               // Player + Volume + Resonance-Source, befüllt in initAudio()
const FINCH_YAW_LINKS  =  Math.PI/2;  // +90° = ganz links → Original-Pitch
const FINCH_YAW_RECHTS = -Math.PI/2;  // -90° = ganz rechts → maximal runtergepitcht
const FINCH_MIN_RATE   = 0.15;        // tiefster Punkt: ~15% Tempo (fast 3 Oktaven tiefer)
const FINCH_DIST       = 2;           // Meter: wie weit vor den Augen der Vogel "hängt"
const AMBI_MIN_RATE    = 0.1;         // Wald-Atmo (ambix) bei "ganz rechts": 10% Tempo
const FINCH_LOOP_MIN   = 0.4;         // kürzeste Loop-Länge (Sek. im Buffer) bei "ganz rechts" –
                                      // schneidet die Stille ab, die Rufe enden bei ~0.35 s
let finchRate = 1;                    // aktueller Wert, fürs HUD
let ambiRate  = 1;                    // aktueller Wert, fürs HUD
let finchLoop = 0;                    // aktuelle Loop-Länge (Sek.), fürs HUD


// ⚠️ DEV-SHORTCUT: true = beim Laden direkt in die Finch-Demo springen (zum
// Arbeiten am Vogel-Pitch, ohne jedes Mal die ganze Experience durchzuspielen).
...
async function initAudio() {
  ...
  // ─── FINCH-DEMO: Vogel-Player laden ───
  // Der Vogel läuft durch eine EIGENE Resonance-Quelle, damit er echt im 3D-Raum
  // sitzt (Abstand, Richtung, Hall). Anders als ein fixes Instrument positionieren
  // wir diese Quelle aber JEDE Frame neu – immer FINCH_DIST Meter entlang der
  // Blickrichtung (siehe tick()). Dadurch bleibt der Vogel beim Kopfdrehen immer
  // vor den Augen, klingt aber mit räumlichem Abstand statt flach im Kopf.
  // loop: true wiederholt das Zwitschern endlos. Startet stumm, eingefadet
  // wird erst in sceneFinch().
  soundsFinch.src    = resonanceScene.createSource();
  soundsFinch.vol    = new Tone.Volume(-Infinity);
  soundsFinch.player = new Tone.Player({ url: '/src/finch.wav', loop: true })
    .connect(soundsFinch.vol);
  soundsFinch.vol.connect(soundsFinch.src.input);
  ...
  if (DEV_SKIP_TO_SCENE5) {
    tick();
    sceneFinch(); // Finch-Demo statt des alten Orchester-Finales (scene5)
    return;
  }
...
const tick = () => {
  ...
  // ─── FINCH-DEMO: Kopfdrehung → Abspielgeschwindigkeit ───
  // Wir übersetzen den yaw-Winkel in einen Wert t von 0 bis 1:
  //   yaw = +90° (ganz links)  → t = 0 → Originaltempo (rate 1)
  //   yaw = -90° (ganz rechts) → t = 1 → FINCH_MIN_RATE (tief & langsam)
  // lerp() clampt t automatisch – dreht man weiter als ±90°, bleibt der Wert
  // einfach am Anschlag stehen statt komische Werte zu erzeugen.
  if (isPlaying && sceneFinchActive) {
    const t = (FINCH_YAW_LINKS - yaw) / (FINCH_YAW_LINKS - FINCH_YAW_RECHTS);
    finchRate = lerp(1, FINCH_MIN_RATE, t);

    // playbackRate direkt jedem Frame neu setzen – bei 60fps sind die Schritte
    // so klein, dass das Ohr keinen Sprung hört, sondern ein stufenloses Gleiten.
    soundsFinch.player.playbackRate = finchRate;

    // ─── Wald-Atmo mit DERSELBEN Bewegung runterpitchen (Test) ───
    // 1 (ganz links) → AMBI_MIN_RATE (ganz rechts). ambiSource ist ein nativer
    // AudioBufferSourceNode → .playbackRate.value setzt Tempo UND Tonhöhe zugleich
    // (Bandmaschinen-Effekt). Alle 16 Ambisonics-Kanäle werden identisch resampled,
    // deshalb bleibt das Klangfeld räumlich kohärent – der Wald wird nur tiefer
    // und träger, die Richtungen bleiben erhalten.
    ambiRate = lerp(1, AMBI_MIN_RATE, t);
    if (ambiSource) ambiSource.playbackRate.value = ambiRate;

    // ─── Loop-Länge an die Drehung koppeln ───
    // finch.wav hat vorne die "Pie-Pie"-Rufe (~0.35 s) und danach eine lange
    // stille Pause bis zum Buffer-Ende (~1.89 s). Wir setzen loopEnd (in Buffer-
    // Sekunden): ganz links = volle Länge → natürliche lange Pause. Je weiter
    // rechts, desto früher springt der Loop zurück (bis FINCH_LOOP_MIN), sodass
    // die Stille wegfällt und die Rufe dicht hintereinander kommen. Tone reicht
    // das live an die laufende Quelle durch. (Die playbackRate streckt das
    // zusätzlich in Echtzeit – beides zusammen ergibt rechts das dichte, tiefe
    // Zwitschern.)
    const finchDur = soundsFinch.player.buffer.duration;
    if (finchDur > 0) {
      finchLoop = lerp(finchDur, FINCH_LOOP_MIN, t);
      soundsFinch.player.loopEnd = finchLoop;
    }

    // ─── Vogel-Position ANS BLICKFELD KOPPELN ───
    // Wir rechnen aus yaw/pitch den Vorwärts-Vektor des Kopfes (gleiche Formel wie
    // bei der Listener-Orientierung oben) und setzen die Quelle FINCH_DIST Meter
    // in genau diese Richtung. Weil der Listener bei (0,0,0) sitzt, heißt das:
    // der Vogel schwebt immer exakt vor den Augen, egal wohin man sich dreht –
    // aber mit echtem räumlichem Abstand statt flach im Kopf.
    //
    // ⚠️ X wird NEGIERT: In diesem Projekt ist die Links/Rechts-Achse zwischen
    // Listener-Orientierung und Quellposition gespiegelt (derselbe Grund, warum
    // schon der Pitch "falschrum" war). Ohne das Minus landet der Vogel auf der
    // falschen Seite (nach links drehen → Vogel rechts). Nur X spiegeln, damit
    // vorne/hinten und die Höhe unberührt bleiben.
    const fwdX =  Math.sin(yaw) * Math.cos(pitch);
    const fwdY =  Math.sin(pitch);
    const fwdZ = -Math.cos(yaw) * Math.cos(pitch);
    soundsFinch.src.setPosition(-FINCH_DIST * fwdX, FINCH_DIST * fwdY, FINCH_DIST * fwdZ);
  }
  ...
  document.getElementById('hud').innerHTML =
    ...
    `rate ${finchRate.toFixed(2)}<br>` +
    `ambi ${ambiRate.toFixed(2)}<br>` +
    `loop ${finchLoop.toFixed(2)}s<br>`;
    // + `HP &nbsp;${headphonesOn}`  
...
function sceneFinch(fadeSek = 1.5) {
  console.log("Wir sind in der Finch-Demo");

  // Wald-Atmosphäre als Klangbett: AmbiNature.wav ist ein Ambisonics-/ambix-File
  // und läuft durch den ambisonicInput (in initAudio verkabelt). fadeInAmbi startet
  // die Quelle und rampt sie über 3 s sanft auf 0.3 hoch. Der Vogel sitzt als
  // eigene Punktquelle davor – so hat man Wald ringsum UND den Vogel vor sich.
  fadeInAmbi(ambiSource, ambiGain, 3, 0.3);

  // Loop starten (läuft ab jetzt endlos) und sanft einfaden statt hart
  // reinzuknallen – rampTo macht aus dem Sprung eine weiche Rampe.
  soundsFinch.player.start();
  soundsFinch.vol.volume.rampTo(2, fadeSek);

  // Ab jetzt übersetzt der tick() jede Kopfdrehung in Pitch UND Vogel-Position.
  sceneFinchActive = true;
}
